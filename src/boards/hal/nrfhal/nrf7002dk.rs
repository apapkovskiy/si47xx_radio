...
//! HAL implementation for nrf7002dk Development Kit.
//!
//! This module provides hardware initialization functions specific to the Nordic
//! Semiconductor nRF7002 Development Kit. It configures all the peripherals needed
//! for the SI47xx radio application:
//!
//! - **Radio Reset**: GPIO P1.03 (active high)
//! - **LED**: GPIO P1.06 (active high)
...
//! # Pin Configuration
//!
//! The pin assignments follow the nRF7002 DK standard pinout and are compatible
...
///
/// Configures GPIO P1.06 as an output with initial low level (LED off, active high).
...
pub fn hal_led_create() -> HalOutput {
    let led = unsafe { peripherals::P1_06::steal() };
...
/// - Write opcode: PP (page program)
/// - Page size: 256 bytes
/// - Pins:
///   - SCK: P0.08 (Clock)
///   - CSN: P0.11 (Chip Select)
///   - IO0-IO1: P0.09-P0.10 (Data lines)
///
...
pub fn hal_qspi_create() -> HalQspi {
    ...
    let csn = unsafe { peripherals::P0_11::steal() };
    let sck = unsafe { peripherals::P0_08::steal() };
    let io0 = unsafe { peripherals::P0_09::steal() };
    let io1 = unsafe { peripherals::P0_10::steal() };
    // The nRF7002 DK has only 2 QSPI data lines, so IO2 and IO3 are not used.
    // but must still be provided to the driver.
    let io2 = unsafe { peripherals::P1_02::steal() };
    let io3 = unsafe { peripherals::P0_27::steal() };
